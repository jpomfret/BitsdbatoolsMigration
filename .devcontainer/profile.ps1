...
Import-Module /workspace/Module/Helper.psm1
#region Setup 
...
# we need an test login for JessP
$Password = ConvertTo-SecureString P@ssword1234 -AsPlainText -Force
New-DbaLogin -SqlInstance $dbatools1 -SqlCredential $continercredential  -Login JessP -SecurePassword $Password -WarningAction SilentlyContinue | Out-Null
New-DbaDbUser -SqlInstance $dbatools1 -SqlCredential $continercredential -Database Pubs -Login JessP -Username JessP -WarningAction SilentlyContinue  | Out-Null
Add-DbaDbRoleMember -SqlInstance $dbatools1 -SqlCredential $continercredential -Database Pubs -User JessP -Role db_owner -Confirm:$false  | Out-Null

# we need an app login for pubs
...
Invoke-DbcCheck -Check InstanceConnection -Show Summary
...
Invoke-DbcCheck -Check DatabaseExists -Show Summary

# dbatools2
$null = Set-DbcConfig -Name app.sqlinstance -Value 'dbatools2'
Invoke-DbcCheck -Check NoDatabasesOn2 -Show Summary

#endregion

# open first demo file
code /workspace/Demos/01_Intro.ps1